...
func (s Service) enforceLoginRateLimit(ctx context.Context, ipAddress string) error {
	...
	key := fmt.Sprintf("rl:%s:%d", ipAddress, windowStart)

	count, err := s.Redis.Incr(ctx, key).Result()
	if err != nil {
		return fmt.Errorf("increment login rate limit: %w", err)
	}
	ttl := time.Duration(windowEnd-now.UnixMilli()+1000) * time.Millisecond
	if ttl > 0 {
		_ = s.Redis.PExpire(ctx, key, ttl).Err()
	}
	if count > int64(maxAttempts) {
		return &requestError{status: http.StatusTooManyRequests, message: "Too many login attempts. Please try again later."}
	}
	return nil
}
