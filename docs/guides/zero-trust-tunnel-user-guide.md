...
- **Docker Engine** for standalone agent containers, **Docker Compose** or **Podman with podman-compose** for generated remote install bundles, or **Go 1.25+** when building a bare-metal agent from source.
...
4. **Run it on the remote host**:
   ...
   ```bash
   umask 077
   mkdir -p arsenale-gateway-<gateway-id>/certs
   cd arsenale-gateway-<gateway-id>
   # paste the full script generated by Arsenale
