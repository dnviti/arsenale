...
Standalone gateway installs live beside each gateway image under `gateways/*/compose.yml`. Tunnel-backed installs (`ssh-gateway`, `guacd`, `db-proxy`, and standalone `tunnel-agent`) require a CLI-generated bundle from `arsenale gateway tunnel-token create <id> --bundle-dir <dir>`, because the tunnel broker requires both token and gateway client certificate material. Generated bundles preserve the gateway's configured listener port in `TUNNEL_LOCAL_PORT`; bundled runtime containers also set the matching listener variable (`GUACD_PORT`, `SSH_PORT`, or `DB_LISTEN_PORT`) so the service and tunnel agent use the same port. Backend tunnel proxy routes try the configured listener first and retain runtime-default fallback ports for already-deployed bundles. GUACD install bundles default to TLS, create local guacd certificate material for the remote container, and declare only named persistence volumes in the top-level Compose `volumes:` map. Tunnel runtimes normalize Arsenale HTTP(S) base URLs to the WebSocket broker endpoint and accept inline PEM or matching `_FILE` PEM variables for CA/client certificate material. `guacenc` remains the direct HTTPS recording conversion service with its own auth token and TLS cert/key mounts.
