...
Standalone gateway installs live beside each gateway image under `gateways/*/compose.yml`. Tunnel-backed installs (`ssh-gateway`, `guacd`, `db-proxy`, and standalone `tunnel-agent`) require a CLI-generated bundle from `arsenale gateway tunnel-token create <id> --bundle-dir <dir>`, because the tunnel broker requires both token and gateway client certificate material. Generated bundles point `TUNNEL_LOCAL_PORT` at the embedded runtime listener (`4822` for `guacd`, `2222` for `ssh-gateway`, `5432` for `db-proxy`) rather than at an externally published host port. Tunnel runtimes normalize Arsenale HTTP(S) base URLs to the WebSocket broker endpoint and accept inline PEM or matching `_FILE` PEM variables for CA/client certificate material. `guacenc` remains the direct HTTPS recording conversion service with its own auth token and TLS cert/key mounts.
