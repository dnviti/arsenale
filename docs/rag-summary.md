...
CLI releases can be installed with `tools/arsenale-cli/install.sh` on Linux/macOS or `tools/arsenale-cli/install.ps1` on Windows. The installers detect OS and CPU architecture, download the matching GitHub release archive, verify it with `checksums_sha256.txt`, install `arsenale` or `arsenale.exe` into a user-writable bin directory, and install shell completions for Bash/Zsh/Fish on Linux/macOS or PowerShell on Windows. `ARSENALE_VERSION=develop` selects the moving `cli-dev` prerelease from the `develop` branch, whose archives use stable `arsenale-cli_develop_<os>_<arch>` names while the binary reports a commit-specific `develop-<sha>` version. The CLI also exposes `arsenale completion bash|zsh|fish|powershell` for manual completion setup.
