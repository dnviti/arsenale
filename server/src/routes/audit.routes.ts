...
router.get('/tenant/gateways', requireTenant, requireTenantRoleAny('ADMIN', 'OWNER', 'AUDITOR'), requirePermission('canViewAuditLog'), asyncHandler(auditController.listTenantGateways));
router.get('/tenant/countries', requireTenant, requireTenantRoleAny('ADMIN', 'OWNER', 'AUDITOR'), requirePermission('canViewAuditLog'), asyncHandler(auditController.listTenantCountries));
router.get('/tenant/geo-summary', requireTenant, requireTenantRoleAny('ADMIN', 'OWNER', 'AUDITOR'), requirePermission('canViewAuditLog'), asyncHandler(auditController.getTenantGeoSummary));
router.get('/tenant', requireTenant, requireTenantRoleAny('ADMIN', 'OWNER', 'AUDITOR'), requirePermission('canViewAuditLog'), validate(tenantAuditQuerySchema, 'query'), asyncHandler(auditController.listTenantLogs));
