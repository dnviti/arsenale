...
export async function getVersionData(req: AuthRequest, res: Response) {
  assertAuthenticated(req);
  const version = parseInt(req.params.version as string, 10);
  if (isNaN(version) || version < 1) {
    throw new AppError('Invalid version number', 400);
...
export async function unshare(req: AuthRequest, res: Response) {
  assertAuthenticated(req);
...
export async function updateSharePermission(req: AuthRequest, res: Response) {
  assertAuthenticated(req);
...
export async function initTenantVault(req: AuthRequest, res: Response) {
  assertAuthenticated(req);
  if (!req.user.tenantId) throw new AppError('Tenant context required', 400);
  if (req.user.tenantRole !== 'OWNER' && req.user.tenantRole !== 'ADMIN') {
    throw new AppError('Only admins and owners can initialize the tenant vault', 403);
  }
...
export async function distributeTenantKey(req: AuthRequest, res: Response) {
  assertAuthenticated(req);
  if (!req.user.tenantId) throw new AppError('Tenant context required', 400);
  if (req.user.tenantRole !== 'OWNER' && req.user.tenantRole !== 'ADMIN') {
    throw new AppError('Only admins and owners can distribute tenant vault keys', 403);
  }
