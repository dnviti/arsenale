...
function getActionColor(action: AuditAction): 'default' | 'primary' | 'secondary' | 'error' | 'warning' | 'success' | 'info' {
  ...
  if (['LOGIN_FAILURE', 'DELETE_CONNECTION', 'DELETE_FOLDER', 'UNSHARE_CONNECTION', 'SECRET_DELETE', 'SECRET_UNSHARE', 'TENANT_DELETE', 'TEAM_DELETE', 'GATEWAY_DELETE', 'TENANT_REMOVE_USER', 'TEAM_REMOVE_MEMBER', 'GATEWAY_UNDEPLOY', 'GATEWAY_TEMPLATE_DELETE', 'SESSION_ERROR', 'SFTP_DELETE', 'SECRET_EXTERNAL_REVOKE', 'SESSION_TERMINATE', 'REFRESH_TOKEN_REUSE'].includes(action)) return 'error';
  ...
  if (['CREATE_CONNECTION', 'CREATE_FOLDER', 'VAULT_SETUP', 'SECRET_CREATE', 'TENANT_VAULT_INIT', 'TENANT_CREATE', 'TEAM_CREATE', 'GATEWAY_CREATE', 'SSH_KEY_GENERATE', 'GATEWAY_TEMPLATE_CREATE', 'SFTP_MKDIR', 'CONNECTION_FAVORITE'].includes(action)) return 'info';
  return 'primary';
}
