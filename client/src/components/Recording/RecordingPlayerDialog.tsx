...
export default function RecordingPlayerDialog({
  ...
  // Asciicast width/height are cols/rows, not pixels.
  const contentWidth = isTextRecording
    ? Math.max((recording.width || 80) * 14, 720)
    : recording.width || 1024;
  const contentHeight = isTextRecording
  ...
  return (
    ...
    >
      <DialogTitle
        sx={{ display: "flex", alignItems: "center", gap: 1, pr: 1, py: 1 }}
      >
        <Typography
          variant="subtitle1"
          component="span"
        ...
        <Chip
          ...
          size="small"
          color={
            protocolColor(recording.protocol) as
              | "success"
              | "primary"
              | "warning"
              | "info"
              ...
              <Table size="small">
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      {analysis.displayWidth} x {analysis.displayHeight}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Sync Frames</TableCell>
                    <TableCell>{analysis.syncCount}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Has Display Data</TableCell>
                    <TableCell>
                      {analysis.hasLayer0Image ? "Yes" : "No"}
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell>Truncated</TableCell>
                    <TableCell>
                      {analysis.truncated ? "Yes (>10MB)" : "No"}
                    </TableCell>
                  </TableRow>
                  {Object.entries(analysis.instructions)
                    ...
                    .map(([op, count]) => (
                      <TableRow key={op}>
                        <TableCell sx={{ pl: 3 }}>{op}</TableCell>
